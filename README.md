...
4. **Build** — `buildCommand` in `render.yaml` is `npm ci --include=dev && npm run import-cedict && npm run build`. If the service was created before that change, update **Settings → Build Command** in the Dashboard to match (git push does not rewrite an existing service’s build command). `tsx` is also a root production dependency so `npm ci` with `NODE_ENV=production` still installs it. With `CEDICT_FETCH=1` the importer downloads the latest CC-CEDICT from MDBG; if that fetch fails it falls back to the repo archive.
